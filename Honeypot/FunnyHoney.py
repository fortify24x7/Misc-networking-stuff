...
class SSHHandler(SocketServer.StreamRequestHandler):
	def handle(self):
		try:
			...
			chan.send(user)
			time.sleep(2)
			cmdline("echo Hello! I installed a reverse shell on your terminal!\r\n",True)
			chan.send("Hello! I installed a reverse shell on your terminal!\r\n")
			time.sleep(0.1)
			chan.send(user)
			time.sleep(1)
			cmdline("echo I also got your root password!\r\n",True)
			time.sleep(0.2)
			chan.send("I also got your root password!\r\n")
			chan.send(user)
			time.sleep(1)
			cmdline("echo I think it is save to say,\"You got owned!\"\r\n")
			time.sleep(0.2)
			chan.send("I think it is save to say,\"You got owned!\"\r\n")
			chan.send(user)
			time.sleep(2)
			cmdline("ls -al")
			time.sleep(0.5)
			...
			chan.send("\r\n-rw-r----- root root  16289372 %s rootload.sh\r\n" %month)
